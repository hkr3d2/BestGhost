...
void PauseLayer::addRaceControls() {
    ...
    m_fields->m_raceToggle = toggle;
    
    // Label for toggle
    auto toggleLabel = CCLabelBMFont::create("Race Ghost", "bigFont.fnt");
    toggleLabel->setPosition({200, 175});
    toggleLabel->setScale(0.6f);
    menu->addChild(toggleLabel);
    
    // Show selected ghost
    this->updateSelectedGhostLabel();
    if (m_fields->m_selectedGhostLabel) {
        m_fields->m_selectedGhostLabel->setPosition({200, 145});
        m_fields->m_selectedGhostLabel->setScale(0.5f);
        menu->addChild(m_fields->m_selectedGhostLabel);
    }
...
void PauseLayer::updateSelectedGhostLabel() {
    ...
    if (activeGhost && GhostManager::get().isRacing()) {
        std::string text = fmt::format("Ghost: {}%", static_cast<int>(activeGhost->percentage));
        if (m_fields->m_selectedGhostLabel) {
            m_fields->m_selectedGhostLabel->setString(text.c_str());
        } else {
            m_fields->m_selectedGhostLabel = CCLabelBMFont::create(text.c_str(), "bigFont.fnt");
        }
    } else if (m_fields->m_selectedGhostLabel) {
        m_fields->m_selectedGhostLabel->setString("Ghost: none");
    }
}
...
void PauseLayer::onToggleRace(CCObject* sender) {
    ...
    if (m_fields->m_raceToggle) {
        m_fields->m_raceToggle->toggle(newState);
    }
...
void PauseLayer::onSelectGhost(CCObject* sender) {
    auto popup = GhostLibraryPopup::create(true);
    popup->show();
}
    ...
    if (GhostManager::get().isRacing()) {
        ...
        if (activeGhost) {
            ...
            if (playLayer) {
                ...
                GhostRenderer::get().start(playLayer->m_gameState.m_time);
            }
        }
    }
